 ...
 *
 * @typedef {'still' | 'video'} WorkflowMediaKind
...
/** Workflow visuals under `public/workflow/` (filenames match card titles where possible). */
const WF_MEDIA = {
  nationalDatabase: "/workflow/workflow-profile.png",
  transferDatabase: "/workflow/Transfer Technology Database.mp4",
  recruitingBoard: "/workflow/Recruiting Board and Depth Charts.mp4",
  prePortal: "/workflow/pre-portal_tracking_system_720.png",
  jucoDatabase: "/workflow/national_juco_database_720.png",
  rosterManagement: "/workflow/roster_management_tools_720.png",
  activityFeed: "/workflow/activity feed still.png",
  onTheRoad: "/workflow/On the Road .mp4",
  scoreTracker: "/workflow/Score Tracker.mp4",
  campData: "/workflow/camp video.mp4",
};
 ...
 */
function encodePublicAssetPath(path) {
  return path
    .split("/")
...
export function resolveWorkflowMediaSrc(media) {
  ...
  return assetPath(encodePublicAssetPath(path));
}
