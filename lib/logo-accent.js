...
export function isNearWhite(r, g, b) {
  const { lum, sat } = colorMetrics(r, g, b);
  return lum > 0.86 && sat < 0.2;
}

/** @param {number} r @param {number} g @param {number} b */
export function isNeutralColor(r, g, b) {
  const { lum, sat } = colorMetrics(r, g, b);
  return sat < 0.14 && lum > 0.1 && lum < 0.86;
}

/**
 * Rank logo pixel colors by frequency (excludes near-white).
 * @param {ImageData} imageData
 * @returns {{ hex: string; count: number; sat: number; lum: number; neutral: boolean }[]}
 */
export function paletteFromImageData(imageData) {
  const { data } = imageData;
  const buckets = new Map();
  ...
  for (let i = 0; i < data.length; i += 4) {
    ...
    const prev = buckets.get(key) || { r: 0, g: 0, b: 0, n: 0, sat: 0, lum: 0 };
    const { sat, lum } = colorMetrics(r, g, b);
    prev.r += r;
    prev.g += g;
    prev.b += b;
    prev.n += 1;
